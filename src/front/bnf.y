...
%token COLON               /* : */
%token COMMA               /* , */
/* TODO: %token HASH                # for lambda */
%token ERROR           /* lexer error */
%token DELIMITER           /* end of line */
%token ENDOFTOKENS         /* end of tokens indicator */

/* keywords */
%token FN RET IF ELSE WHL FOR SWITCH BRK DEFAULT CLASS TOSS DEL ENUM OP

%token <modifier> CONST LOCAL VAR STATIC HIDDEN
...
/* variable declaration */
    ...
    var_decl_modifiers
        : var_decl_modifiers var_decl_modifier
            {
                $$ = static_cast<Modifier>(to_underlying($1) | to_underlying($2));
            }
        |
            {
                $$ = Modifier::None;
...
/* class definition */
    ...
    class_member
        ...
        | class_member_mod_list fn_def
            ...
            /*
        | class_member_mod_list class_op_def
            {
                $$ = newClassMethodDef($1, $2);
            }
            */
...
/* TODO:
    class_op_def
        : OP nameNode class_op_def_operators LPAREN fn_opt_params_list RPAREN code_block
            {
                $$ = newClassOpDef($2->data.stringValue, $3, $5, $7);
            }
    ;

    class_op_def_operators
        : op_cond
            {
                $$ = $1;
            }
...
*/
    ...
    class_member_mod_list
        : class_member_mod_list class_member_mod
            {
                $$ = static_cast<Modifier>(static_cast<uint16_t>($1) | static_cast<uint16_t>($2));
            }
        |
            {
                $$ = Modifier::None;
